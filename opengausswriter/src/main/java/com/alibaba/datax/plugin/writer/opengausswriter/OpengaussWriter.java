package com.alibaba.datax.plugin.writer.opengausswriter;
 ...
 * @date 2023/3/28
 */
public class OpengaussWriter extends Writer {
    ...
    public static class Job extends Writer.Job {
        ...
        public void init() {
            ...
            // warn：not like mysql, Opengauss only support insert mode, don't use
            String writeMode = this.originalConfig.getString(Key.WRITE_MODE);
            if (null != writeMode) {
                throw DataXException.asDataXException(DBUtilErrorCode.CONF_ERROR,
                        String.format("写入模式(writeMode)配置有误. 因为Opengauss不支持配置参数项 writeMode: %s, Opengauss仅使用insert sql 插入数据. 请检查您的配置并作出修改.", writeMode));
            }
