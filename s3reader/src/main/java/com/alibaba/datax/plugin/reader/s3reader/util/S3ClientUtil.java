...
public class S3ClientUtil {
    ...
    public static S3Client initOssClient(Configuration conf) {
        ...
        URI s3URI = URI.create(endpoint);

        AwsBasicCredentials awsCredentials = AwsBasicCredentials.create(accessKey, secretKey);
        return S3Client.builder()
                .credentialsProvider(StaticCredentialsProvider.create(awsCredentials))
                .region(Region.of("ignored")).endpointOverride(s3URI)
                .build();
    }
    ...
    public static ResponseInputStream<GetObjectResponse> getObject(S3Client s3Client, String bucket, String object) {
        ...
        try {
            ...
            log.error("s3 file {} get failed", object);
            throw DataXException.asDataXException(S3ReaderErrorCode.S3_EXCEPTION, e.getMessage());
        }
    }

    public static ResponseInputStream<GetObjectResponse> getObjectAfterDone(S3Client s3Client, String bucket, String object) {
        GetObjectRequest request = GetObjectRequest.builder()
                .bucket(bucket)
                .key(object)
                .build();
        try {
            ...
            log.info("s3 file {} get failed! Cause haveDoneObject is true, task continue", object);
            return null;
        }
    }
