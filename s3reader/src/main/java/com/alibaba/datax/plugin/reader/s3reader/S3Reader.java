...
public class S3Reader extends Reader {
		...
		private void validate() {
			...
			if (StringUtils.isBlank(accessKey)) {
				throw DataXException.asDataXException(
						S3ReaderErrorCode.CONFIG_INVALID_EXCEPTION, "您需要指定 accessKey");
			}

			String secretKey = this.readerOriginConfig.getString(Key.SECRETKEY);
			if (StringUtils.isBlank(secretKey)) {
				throw DataXException.asDataXException(
						S3ReaderErrorCode.CONFIG_INVALID_EXCEPTION, "您需要指定 secretKey");
			}
			...
			if (StringUtils.isBlank(endpoint)) {
				throw DataXException.asDataXException(S3ReaderErrorCode.CONFIG_INVALID_EXCEPTION, "您需要指定 endpoint");
			}
			...
			if (null == fieldDelimiter || fieldDelimiter.length() == 0) {
				throw DataXException.asDataXException(
						S3ReaderErrorCode.CONFIG_INVALID_EXCEPTION, "您需要指定 fieldDelimiter");
			}
			...
			boolean haveDoneObject = this.readerOriginConfig.getBool(Key.HAVEDONEOBJECT, false);
			if (haveDoneObject) {
				String doneObject = this.readerOriginConfig.getString(Key.DONEOBJECT);
				if (StringUtils.isBlank(doneObject)) {
					throw DataXException.asDataXException(S3ReaderErrorCode.CONFIG_INVALID_EXCEPTION, "您需要指定 doneObject");
				}

				int doneCheckLevel = this.readerOriginConfig.getInt(Key.DONECHECKLEVEL, 1);
				if (doneCheckLevel == 1) {
					LOG.info("doneCheckLevel is 1, s3 doneObject file need available");
					S3Client s3Client = S3ClientUtil.initOssClient(this.readerOriginConfig);
					ResponseInputStream<GetObjectResponse> doneInputStream = S3ClientUtil.getObject(s3Client, bucket, doneObject);
				} else if (doneCheckLevel == 2) {
					LOG.info("doneCheckLevel is 2, s3 doneObject file doesn't need available");
					S3Client s3Client = S3ClientUtil.initOssClient(this.readerOriginConfig);
					ResponseInputStream<GetObjectResponse> doneInputStream = S3ClientUtil.getObjectAfterDone(s3Client, bucket, doneObject);
					if (doneInputStream == null) {
						this.readerOriginConfig.set(Key.ISDONEEXIST, false);
					} else {
						this.readerOriginConfig.set(Key.ISDONEEXIST, true);
					}
				} else {
					throw DataXException.asDataXException(S3ReaderErrorCode.ILLEGAL_VALUE, "doneCheckLevel值目前只支持1、2");
				}
				LOG.info("s3 doneObject file check finish!");
			}
	...
	public static class Task extends Reader.Task {
		...
		public void startRead(RecordSender recordSender) {
			...
			boolean haveDoneObject = readerSliceConfig.getBool(Key.HAVEDONEOBJECT, false);

			ResponseInputStream<GetObjectResponse> inputStream = null;
			if (haveDoneObject) {
				int doneCheckLevel = readerSliceConfig.getInt(Key.DONECHECKLEVEL, 1);

				if (doneCheckLevel == 1) {
					inputStream = S3ClientUtil.getObjectAfterDone(s3Client, bucket, object);
				} else if (doneCheckLevel == 2) {
					boolean isDoneExist = readerSliceConfig.getBool(Key.ISDONEEXIST);
				...
				inputStream = S3ClientUtil.getObject(s3Client, bucket, object);
			}
