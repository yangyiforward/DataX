...
public class HdfsHelper {
    public static final Logger LOG = LoggerFactory.getLogger(Hdfs3xWriter.Job.class);
    ...
    public void getFileSystem(String defaultFS, Configuration taskConfig) {
        ...
        try {
            ...
            LOG.error(message);
            throw DataXException.asDataXException(Hdfs3xWriterErrorCode.CONNECT_HDFS_IO_ERROR, e);
        } catch (Exception e) {
        ...
        if (null == fileSystem || null == conf) {
            String message = String.format("获取FileSystem失败,请检查HDFS地址是否正确: [%s]",
                    "message:defaultFS =" + defaultFS);
            LOG.error(message);
            throw DataXException.asDataXException(Hdfs3xWriterErrorCode.CONNECT_HDFS_IO_ERROR, message);
        }
    }
    ...
    private void kerberosAuthentication(String kerberosPrincipal, String kerberosKeytabFilePath) {
        if (haveKerberos && StringUtils.isNotBlank(this.kerberosPrincipal) && StringUtils.isNotBlank(this.kerberosKeytabFilePath)) {
            ...
            try {
                ...
                LOG.error(message);
                throw DataXException.asDataXException(Hdfs3xWriterErrorCode.KERBEROS_LOGIN_ERROR, e);
            }
        }
    }
    ...
    public void renameFile(HashSet<String> tmpFiles, HashSet<String> endFiles) {
        ...
        if (tmpFiles.size() != endFiles.size()) {
            ...
            LOG.error(message);
            throw DataXException.asDataXException(Hdfs3xWriterErrorCode.HDFS_RENAME_FILE_ERROR, message);
        } else {
            try {
    ...
    public FSDataOutputStream getOutputStream(String path) {
        ...
        try {
            ...
            String message = String.format("Create an FSDataOutputStream at the indicated Path[%s] failed: [%s]",
                    "message:path =" + path);
            LOG.error(message);
            throw DataXException.asDataXException(Hdfs3xWriterErrorCode.Write_FILE_IO_ERROR, e);
        }
            ...
            switch (columnType) {
                ...
                default:
                    throw DataXException
                            .asDataXException(
                                    Hdfs3xWriterErrorCode.ILLEGAL_VALUE,
                                    String.format(
    ...
    public void parquetFileStartWrite(RecordReceiver lineReceiver, Configuration config, String fileName,
        ...
        if(!"UNCOMPRESSED".equalsIgnoreCase(compress) && null != compress ) {
            Class<? extends CompressionCodec> codecClass = getCompressCodec(compress);
            if (null != codecClass) {
                outFormat.setOutputCompressorClass(conf, codecClass);
            }
        ...
        try {
            ...
            columns.forEach(col ->colTypes.add(col.getString(Key.TYPE).toLowerCase()));
